 ...
 * every one formatting with `toFixed` - which hardcodes the decimal point and prints `1.5 MB` even
 * where the interface's own locale spells that number `1,5 MB`. `Intl` takes the separator from
 * the locale instead.
 *
...
export function formatBytes(bytes: number): string {
	...
	const formatted = new Intl.NumberFormat('de-DE', {
		...
		// Grouping off. `de-DE` groups thousands with a point and separates decimals with a comma,
